...
for j in range(Nq):
    if df["Tipo"][j]=="checkbox":
        ...
        pregunta.append('    <a href="#" data-toggle="tooltip" title="Ayudaaaaa"><i class="fa fa-question-circle" style=font-size:24px></i></a><br>')
        pregunta.append('  </div>')
        pregunta.append('</div>')
        pregunta.append('<div class="row">')
        ...
        for k in range(len(respuestas)):
            if respuestas[k]=='Otro' or respuestas[k]=='Otros:':
                l=l+1
                pregunta.append('    <label><input type="checkbox" class="option-input checkbox" id="question'+str(j+1)+'" name="cboxq'+str(j+1)+'" value="'+str(k+1)+'" onclick="var input=document.getElementById("name'+str(l)+'"); if(this.checked){ input.disabled = false; input.focus();}else{input.disabled=true;}" />Otro<br><input id=" name'+str(l)+'" name=" name'+str(l)+'"/></label><br>')
                print(pregunta[-1])
            else:
        ...
        for k in range(len(respuestas)):
            if respuestas[k]=='Otro' or respuestas[k]=='Otros:':
                l=l+1
                pregunta.append('    <input type="radio" class="option-input radio" id="question'+str(k+1)+'" name="question'+str(j+1)+'" value="'+str(k+1)+'" onclick="var input = document.getElementById("name'+str(l)+'"); if(this.checked){ input.disabled = false; input.focus();}else{input.disabled=true;}" />Otro<br><input id="name'+str(l)+'" name="name'+str(l)+'/><br>')
            else:
        ...
        for k in range(len(respuestas)):
            if respuestas[k]=='Otro' or respuestas[k]=='Otros:':
                ...
                pregunta.append('  </div>')
                pregunta.append('  <div class="col-xs-1 col-sm-1 col-md-1 col-lg-1">')
